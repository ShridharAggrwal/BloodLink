...
const ViewRequests = () => {
  ...
  const fetchRequests = async () => {
    try {
      // Use the common alerts endpoint which includes accepted requests with availability message
      ...
      {requests.length === 0 ? (
        ...
        <div className="grid gap-4">
          {requests.map((request) => (
            <div key={request.id} className={`bg-white rounded-2xl p-6 border shadow-sm hover:shadow-lg transition-all duration-300 ${request.is_accepted ? 'border-amber-200 hover:border-amber-300' : 'border-slate-200 hover:border-rose-200'}`}>
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div className="flex items-start gap-4">
                  <div className={`w-14 h-14 rounded-xl flex items-center justify-center flex-shrink-0 shadow-lg ${request.is_accepted ? 'bg-gradient-to-br from-amber-400 to-orange-500 shadow-orange-200' : 'bg-gradient-to-br from-rose-500 to-red-600 shadow-red-200'}`}>
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  ...
                  <button
                    ...
                    disabled={accepting || request.is_accepted}
                    className={`px-5 py-2.5 rounded-xl font-medium shadow-md transition-all disabled:opacity-50 ${request.is_accepted
                  ...
                  >
                    {request.is_accepted ? 'Accepted' : 'Fulfill'}
                  </button>
                </div>
              </div>
              ...
              {request.is_accepted && (
                <div className="mt-3 bg-amber-50 border border-amber-200 rounded-lg p-2.5 text-xs text-amber-700">
                  ⏳ {request.availability_message || 'This request has been accepted but not yet fulfilled. It may become available again if help is not completed.'}
                </div>
              )}
            </div>
          ))}
...
const AppointmentsManagement = () => {
  ...
  const createDefaultSlots = async () => {
    setSaving(true)
    try {
      // Create slots for all active days (Mon-Sat by default)
      const slots = []
      ...
      await api.post('/appointments/bank-defaults', { slots })
      setToast({ type: 'success', message: 'Default slots created successfully!' })
      fetchData()
    } catch (error) {
      setToast({ type: 'error', message: 'Failed to create default slots' })
    } finally {
      setSaving(false)
    }
  }

  const toggleDayActive = async (dayOfWeek) => {
    setSaving(true)
    try {
      // Get current slots
      const currentDaySlots = defaultSlots.filter(s => s.day_of_week === dayOfWeek)

      if (currentDaySlots.length > 0) {
        // Remove this day's slots
        const newSlots = defaultSlots.filter(s => s.day_of_week !== dayOfWeek)
        await api.post('/appointments/bank-defaults', { slots: newSlots })
        setToast({ type: 'success', message: `${daysOfWeek[dayOfWeek]} marked as holiday` })
      } else {
        ...
        await api.post('/appointments/bank-defaults', { slots: newSlots })
        setToast({ type: 'success', message: `${daysOfWeek[dayOfWeek]} is now active` })
      }
      fetchData()
    } catch (error) {
  ...
  const updateBookingStatus = async (id, status) => {
    try {
      ...
      setToast({ type: 'success', message: `Booking ${status}` })
      fetchData()
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-500">Manage your appointment slots and view bookings</p>
      </div>
      ...
      <div className="flex gap-2 p-1 bg-white border border-slate-200 rounded-xl w-fit shadow-sm">
        <button
          ...
          className={cn(
            "px-5 py-2.5 rounded-lg transition-all font-medium text-sm flex items-center gap-2",
            activeTab === 'slots' ? 'bg-slate-900 text-white' : 'text-slate-500 hover:bg-slate-50'
          )}
        >
        ...
        <button
          ...
          className={cn(
            "px-5 py-2.5 rounded-lg transition-all font-medium text-sm flex items-center gap-2",
            activeTab === 'bookings' ? 'bg-slate-900 text-white' : 'text-slate-500 hover:bg-slate-50'
          )}
        >
        ...
        <div className="space-y-6">
          ...
          {defaultSlots.length === 0 ? (
            <div className="bg-white border border-slate-200 rounded-2xl p-8 text-center shadow-sm">
              ...
              <h3 className="text-lg font-semibold text-slate-800 mb-2">No Schedule Set Up</h3>
              <p className="text-slate-500 mb-6 max-w-md mx-auto">Create a default weekly schedule so users can book appointments at your blood bank.</p>
              <button
                onClick={createDefaultSlots}
                disabled={saving}
                className="px-6 py-3 bg-rose-600 hover:bg-rose-700 text-white rounded-xl font-medium shadow-lg shadow-rose-100 transition-all flex items-center gap-2 mx-auto disabled:opacity-50"
              >
            ...
            <>
              ...
              <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
                <div className="flex items-center justify-between mb-6">
                  ...
                  <p className="text-xs text-slate-500">Click a day to toggle as holiday</p>
                </div>

                <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-7 gap-3">
                  {daysOfWeek.map((day, index) => {
                    const daySlots = defaultSlots.filter(s => s.day_of_week === index)
                      ...
                      <button
                        ...
                        className={cn(
                          ...
                          isActive
                            ...
                            : "bg-slate-50 border-slate-200 hover:border-rose-300 opacity-60"
                        )}
                      >
                        <p className={cn(
                        ...
                        {isActive ? (
                          <p className="text-xs text-emerald-600">{daySlots.length} slots</p>
                        ) : (
                          <p className="text-xs text-slate-400 flex items-center justify-center gap-1">
                            <CalendarX className="w-3 h-3" /> Holiday
                          </p>
                        )}
                      </button>
                    )
              ...
              {/* Time Slots Preview */}
              <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
                <h2 className="text-lg font-bold text-slate-800 mb-4">Active Time Slots</h2>
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-2">
                  {defaultTimeSlots.map((slot, idx) => (
                    <div key={idx} className="bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-center">
                      <p className="text-sm font-medium text-slate-700">{formatTime(slot.start_time)}</p>
                      <p className="text-xs text-slate-400">to {formatTime(slot.end_time)}</p>
                    </div>
                  ))}
                </div>
                <p className="text-xs text-slate-500 mt-3">These slots apply to all active days. Max 5 bookings per slot.</p>
              </div>
            </>
          )}
        </div>
        ...
        <div className="space-y-4">
          {bookings.length === 0 ? (
            <div className="bg-white border border-slate-200 rounded-2xl p-8 text-center shadow-sm">
              ...
              <p className="text-slate-500">When users book appointments, they'll appear here.</p>
            </div>
          ) : (
            bookings.map((booking) => (
              <div key={booking.id} className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div className="flex items-start gap-4">
                    <div className={cn(
                      ...
                      <span className="text-lg font-bold text-slate-700">{booking.blood_group}</span>
                    </div>
                    <div>
...
const BloodBankDashboard = () => {
  ...
  const navItems = [
    ...
    { path: '/blood-bank/appointments', label: 'Appointments', icon: Calendar },
    { path: '/blood-bank/request-blood', label: 'Request Blood', icon: Megaphone },
    { path: '/blood-bank/history', label: 'History', icon: History },
